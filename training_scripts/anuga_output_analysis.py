...
def animate_water_level_video(
    ...
    if clim is not None:
        ...
        vmin = float(np.nanquantile(st_faces_all, 0.001))
        vmax = float(np.nanquantile(st_faces_all, 0.999))
...
def animate_depth_video(
    ...
    print("Depth video colorbar limits:",
      np.nanquantile(d0_faces, 0.05),
      np.nanquantile(d0_faces, 0.95))
...
def main():
    ...
    "--export-all-nodes",
    action="store_true",
    help="Export depth time series for all nodes to CSV"
)
    ...
    max_depth = depth.max(axis=0)                                # (N,)
        # Optional: export depth time series for ALL nodes
    if args.export_all_nodes:
        import csv
        allnodes_path = outdir / "depth_timeseries_all_nodes.csv"
        print(f"Writing depth time series for all nodes → {allnodes_path}")

        T, N = depth.shape
        with open(allnodes_path, "w", newline="") as f:
            w = csv.writer(f)
            # Header: node_id, X, Y, depth at each time
            header = ["node_id", "X", "Y"] + [f"t={float(tt):.1f}s" for tt in t]
            w.writerow(header)

            for i in range(N):
                row = [i, float(x[i]), float(y[i])] + [float(val) for val in depth[:, i]]
                w.writerow(row)
